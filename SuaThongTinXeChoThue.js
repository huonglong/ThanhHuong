...
import {StatusBar, StyleSheet, Text, View, Alert, TextInput, TouchableOpacity, ActivityIndicator,FlatList, Picker, ToastAndroid} from 'react-native';
...
export default class DoiMatKhau extends Component{
  ...
  componentDidMount(){
    this.setState({tenXe:this.props.navigation.state.params.thamSoTenXe})
    this.setState({tuyChonThue:this.props.navigation.state.params.thamSoHinhThuc})
    this.setState({id: this.props.navigation.state.params.thamSoID})
    this.setState({bienSo: this.props.navigation.state.params.thamSoBienSo})
    this.setState({nguoiThue: this.props.navigation.state.params.thamSoTenKH})
    this.setState({CMND: this.props.navigation.state.params.thamSoCMND})
    this.setState({ngayThue: this.props.navigation.state.params.thamSoNgayThue})
    this.setState({gioThue: this.props.navigation.state.params.thamSoGioThue})
    this.setState({giaThue: this.props.navigation.state.params.thamSoGiaThue})
  }
  ...
  saveData(){
    ...
    // Alert.alert("Thong tin", this.state.CMND + "-" + this.state.tenXe + "-" + this.state.bienSo + "-" + this.state.nguoiThue + "-" +this.state.ngayThue + "-"+this.state.gioThue +"-"+ this.state.giaThue + "-" +this.state.tongTien)
    // return
     this.setState({ ActivityIndicator_Loading : true }, () =>
     {
        fetch('http://192.168.235.2/API_Project/Update_XeThue.php',
        {
            method: 'POST',
            headers: 
            {
      ...
      <View style={styles.comtainer}>
        ...
        <StatusBar hidden ={true}/>
            ...
            <View style = {styles.containerHeader}>
                <Text style = {styles.headerText}>SỬA THÔNG TIN XE CHO THUÊ</Text>
            ...
            <View style={styles.containerTextAndTextInput}>
              <Text style={styles.TextLoiNhac}>Tên xe: </Text>
                <Picker style = {{width: '75%', marginTop: -25,}} 
                  itemTextStyle={{fontSize: 12, fontFamily: 'Times New Roman'}}
            ...
            {/* Hien thi tong tien */}
            <View style={styles.containerTextAndTextInput}>
              <Text style={styles.TextLoiNhac}>Tổng tiền: </Text>
              <Text style= { styles.TextInputStyleClass }>{this.props.navigation.state.params.thamSoTongTien}</Text>
            </View>
